<?php
declare(strict_types=1);
require_once __DIR__ . '/common.php';

handle_request(function (PDO $pdo) {
  require_fetch_request();

  $body     = read_json_body();
  $clientId = client_id_or_fail($body);

  $author = clean_text((string)($body['author'] ?? ''), 40);
  if ($author === '') $author = 'guest';

  $message = clean_text((string)($body['body'] ?? ''), 500);
  if ($message === '') json_error('message is empty', 400);

  // --- basic flood control, keyed off the client id (not an IP, since
  // shared/mobile networks make IP-based limits too easy to false-positive) ---
  $stmt = $pdo->prepare('SELECT created_at FROM messages WHERE client_id = ? ORDER BY id DESC LIMIT 1');
  $stmt->execute([$clientId]);
  $last = $stmt->fetchColumn();
  if ($last !== false) {
    $lastTime = new DateTime((string)$last);
    $now = new DateTime();
    $elapsed = ($now->getTimestamp() - $lastTime->getTimestamp())
      + (((int)$now->format('u') - (int)$lastTime->format('u')) / 1_000_000);
    if ($elapsed < 1.5) json_error('slow down — sending too fast', 429);
  }

  $stmt = $pdo->prepare(
    'SELECT COUNT(*) FROM messages WHERE client_id = ? AND created_at > (NOW(6) - INTERVAL 1 MINUTE)'
  );
  $stmt->execute([$clientId]);
  if ((int)$stmt->fetchColumn() >= 20) {
    json_error('slow down — too many messages, try again in a bit', 429);
  }

  // --- insert (prepared statement — $author/$message/$clientId are bound
  // parameters, never concatenated into the SQL string) ---
  $stmt = $pdo->prepare('INSERT INTO messages (author, body, client_id, created_at) VALUES (?, ?, ?, NOW(6))');
  $stmt->execute([$author, $message, $clientId]);
  $id = (int)$pdo->lastInsertId();

  $stmt = $pdo->prepare('SELECT id, author, body, client_id, created_at FROM messages WHERE id = ?');
  $stmt->execute([$id]);
  $row = $stmt->fetch();

  json_out(['message' => $row]);
});
